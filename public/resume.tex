...
\begin{center}
  ...
  \href{https://linkedin.com/in/feolu-kolawole}{LinkedIn} $\cdot$
  \href{https://feolukolawole.com}{feolukolawole.com} $\cdot$
  \href{https://github.com/FeoluK}{GitHub} $\cdot$
  \href{https://scholar.google.com/citations?user=--VbwJ0AAAAJ}{Google Scholar}
\end{center}
...
\textbf{Anthropic} \hfill \textbf{Berkeley, CA}\\
\textit{Research Fellow (Advised by Monte MacDiarmid)} \hfill \textit{Aug 2026 - Present}
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt, parsep=0pt]
  \item Designing specialized \textbf{reinforcement learning} environments to study how post-training shapes model behavior.
  \item Applying \textbf{mechanistic interpretability} techniques to analyze learned representations and training dynamics.
\end{itemize}

\smallskip

\textbf{Microsoft} \hfill \textbf{Redmond, WA}\\
\textit{Machine Learning Researcher (Advised by Dr. Olesya Melnichenko)} \hfill \textit{Jun 2026 - Aug 2026}
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt, parsep=0pt]
  \item Trained a \textbf{computer vision} model that automates MRI scan calibration, cutting calibration overhead by \textbf{$\sim$40\%}.
  \item Developed a \textbf{sequence file visualization} program that renders MRI undersampling patterns, enabling
  acquisition protocols to be inspected and iterated on before any physical scan is run.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt, parsep=0pt]
  \item Drove CNN models generating realistic 3D hair reconstructions across demographics, boosting accuracy to \textbf{96\%}.
  \item Built a robust extraction pipeline using OpenCV and SAM, achieving perfect segmentation on \textbf{98\%} of images.
\end{itemize}

\smallskip

\textbf{Stanford Human Perception Lab} \hfill \textbf{Palo Alto, CA}\\
\textit{Machine Learning Researcher (PI - Khizer Khaderi)} \hfill \textit{Dec 2024 - May 2026}
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt, parsep=0pt]
...
\textbf{Cross-Game Semantic Alignment of Latent Action Representations}
\href{https://feolukolawole.com/Cross-Game_Semantic_Alignment.pdf}{\textcolor{blue}{(IJCAI GLOW 2026 -- Best Paper Candidate)}}
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt, parsep=0pt]
  \item Showed a single inverse dynamics model trained across racing, Atari, and first-person games clusters action embeddings by \textbf{action class rather than game identity}, with 50 labels per class recovering \textbf{91.1\%} accuracy.
\end{itemize}
